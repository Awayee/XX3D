...
VulkanRHIRenderPass::VulkanRHIRenderPass(const RHIRenderPassDesc& desc, VulkanDevice* device): RHIRenderPass(desc), m_RenderPass(VK_NULL_HANDLE), m_Framebuffer(VK_NULL_HANDLE), m_Device(device){}

VulkanRHIRenderPass::~VulkanRHIRenderPass() {
	DestroyHandle();
}

void VulkanRHIRenderPass::SetName(const char* name) {
	VK_SET_OBJECT_NAME(VK_OBJECT_TYPE_RENDER_PASS, m_RenderPass, name);
}

void VulkanRHIRenderPass::ResolveImageLayout(const VulkanImageLayoutMgr* layoutMgr) {
...
void VulkanRHIRenderPass::DestroyHandle() {
...
void VulkanRHIRenderPass::CreateHandle() {
	ASSERT(m_ImageLayoutMgr, "VulkanRHIRenderPass::CreateHandle() null image layout mgr!");
	...
	for (; i < m_Desc.ColorTargets.Size(); ++i) {
		...
		VulkanRHITexture* vkTex = dynamic_cast<VulkanRHITexture*>(rtInfo.Target);
	...
	if(depthInfo.Target) {
		...
		VulkanRHITexture* vkTex = dynamic_cast<VulkanRHITexture*>(depthInfo.Target);
		attachments[i] = vkTex->GetView();
		++i;
	}
	...
	vkCreateRenderPass(m_Device->GetDevice(), &passInfo, nullptr, &m_RenderPass);
	...
	framebufferInfo.width = m_Desc.RenderArea.w;
	framebufferInfo.height = m_Desc.RenderArea.h;
	framebufferInfo.layers = 1;//TODO multi sampling
	vkCreateFramebuffer(m_Device->GetDevice(), &framebufferInfo, nullptr, &m_Framebuffer);
}