...
VkImageLayout ToVkImageLayout(RImageLayout layout) {
	static VkImageLayout s_ImageLayoutMap[RImageLayout::IMAGE_LAYOUT_MAX_ENUM] = {
...
inline VkBufferUsageFlags ToBufferUsage(EBufferFlags flags) {
	...
	if (flags & EBufferFlagBit::BUFFER_FLAG_TPY_DST) {
...
inline VkMemoryPropertyFlags ToBufferMemoryProperty(EBufferFlags flags) {
	if (flags & (EBufferFlagBit::BUFFER_FLAG_CPY_SRC | EBufferFlagBit::BUFFER_FLAG_UNIFORM | EBufferFlagBit::BUFFER_FLAG_STORAGE)) {
...
inline VkCompareOp ToVkCompareOp(ECompareType type) {
	switch (type) {
...
inline VkBlendFactor ToBlendFactor(EBlendFactor factor) {
	switch(factor) {
