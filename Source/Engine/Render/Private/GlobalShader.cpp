...
namespace Render {
	GlobalShader::GlobalShader(const XString& shaderFile, const XString& entry, EShaderStageFlags type, TConstArrayView<ShaderCompiler::Macro> macros, uint32 permutationID) {
		...
		if (!ShaderCompiler::LoadCompiledShader(outputFile, codeData)) {
			...
			if (ShaderCompiler::CompileSourceFileToFile(shaderFile, entry, type, macros, permutationID)) {
				...
				LOG_FATAL("Failed to compile shader: %s, %s", shaderFile.c_str(), entry.c_str());
			}
		}
